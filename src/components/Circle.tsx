...
const Circle: React.FC<ShapeProps> = ({ shape, updateShape }) => {
  ...
  const handleMouseDown = (event: React.MouseEvent<HTMLDivElement, MouseEvent>, resize: boolean = false) => {
    if ((event.target as HTMLDivElement).className === "resizer") resize = true;
  ...
  const handleMouseMove = (event: MouseEvent) => {
    if (isDragging && startPos) {
      ...
      const dx = event.clientX - startPos.x;
      const dy = event.clientY - startPos.y;
      updateShape({ ...shape, size: Math.max(100, shape.size + Math.max(dx, dy)) });
      setStartPos({ x: event.clientX, y: event.clientY });
    }
  };
